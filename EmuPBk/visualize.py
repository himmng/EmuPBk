...
<<<<<<< HEAD
        ...
        camera = Camera(fig)
        for i in range(len(self.xh_test)):
            plt.title('Simulated vs. ANN Prediction of EoR 21-cm Powerspectrum',
                        size=18, color='Darkslategray')
            plt.xlabel('$k(Mpc^{-1})$', size=20, color='Gray')
            plt.ylabel(r'$k^3P(k)/2\pi^2(mK^2)$', size=20, color='gray')
            plt.xscale('log')
            plt.ylim(0, int(np.max(self.test_data)+5))
            plt.yscale('symlog')
            plt.plot(self.k, self.pk_pred[i], marker='o',ls='-',color= 'blue',lw=3)
            plt.plot(self.k, self.test_data[i], ls='--',lw=2,color='red' )
            plt.legend([r'ANN Prediction',
                            'Powerspectrum for $\zeta$ = {0:.2f}, '
                            'Rmfp = {1:.2f}, $Mhmin(10^8 M\odot)$ = {2:.0f}, xH1 = {3:.3f}'
